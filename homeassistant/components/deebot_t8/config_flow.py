...
from .const import (
    ...
    ATTR_COUNTRY, ATTR_PASSWORD, ATTR_USERNAME, ATTR_PASSWORD_HASH,
    ATTR_DEVICE_ID)
...
STEP_USER_DATA_SCHEMA = vol.Schema(
    {
        ...
        vol.Required(ATTR_CONTINENT): vol.In(SERVER_CONTINENTS),
    }
)
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[
    ...
    try:
        await hass.async_add_executor_job(
            auth_client.login, username, password_hash
        )
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        try:
            ...
            return self.async_create_entry(title="Ecovacs DEEBOT T8",
                                           data=data)
